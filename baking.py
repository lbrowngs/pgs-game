...
class baking():
   ...
   def bakingAisle(self):
        ...
        while flourChoice not in ["A", "B", "C"]:
            flourChoice = input("Please make a selection, A, B, or C.: ")
            flourChoice = flourChoice.upper()

        if flourChoice == "A":
            input(f"{name} deftly dodges the flour hoarder and snatches the last bag of flour, fleeing before the grumpy man can confront them. ")
        elif flourChoice == "B":
            input(f"{name} turns toward the flour hoarder and points out the sign that says '1 bag per customer.' \nThe man explodes with rage and grabs the last bag of flour from the shelf, throwing it at {name}. ")
            conf.change(-10)
        elif flourChoice == "C":
            input(f"{name} quickly grabs the last bag of flour and finds an employee further down the aisle. \nThey alert the employee to the flour hoarding situation, and are thanked for their efforts. ")
            conf.change(3)
        ...
        # VANILLA SCENARIO
        input(f"{name} now must go grab some very specific vanilla that their S.O. has strictly asked for. ")
        ...
        vanChoice = input("What should they do? \nA: Grab the first vanilla they see. \nB: Keep looking and find the vanilla their S.O. asked for. \nAnswer: ")
        vanChoice = vanChoice.upper()

        while vanChoice not in ["A", "B"]:
            vanChoice = input("Please make a selection, A or B.: ")
            vanChoice = vanChoice.upper()

        if vanChoice == "A":
            input(f"{name} quickly grabs the first vanilla they see, rushing out of the aisle. \nBut they can't stop thinking about their S.O.'s disappointed face.")
            conf.change(-3)
        elif vanChoice == "B":
            input(f"{name} continues looking, feeling the long stare of impatience from the person waiting behind them. \n{name} finally finds the correct vanilla and gets out of the aisle.")
            conf.change(-2)


        #DONE WITH BAKING AISLE
        input(f"{name} now has both flour and vanilla, and can head to a different aisle.")